...
import CardItemValveSequenceList from './CardItemValveSequence';

const ValveSequenceListScreen = ({ route }) => {
    ...
    const handleCardItemPress = (item) => {
        ...
        console.log(item)
        navigation.navigate('ValveSequenceScreen', {
    ...
    const fetchData = async (id) => {
        try {
            ...
            let data = response.data.filter(x=>x.SequenceNo==10)
            console.log(data)
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <Icon onPress={handleBack} name="chevron-left" size={30} color="green" />
                ...
                <FlatList
                    data={formDataList}
                    renderItem={({ item }) => <CardItemValveSequenceList item={item}
...
export default ValveSequenceListScreen;